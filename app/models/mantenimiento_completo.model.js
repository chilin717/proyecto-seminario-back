
const mongoose=require('mongoose');

const mantenimiento_CompletoSchema = mongoose.Schema({
    id:{
        type:Int32Array,
        min:1,
        require:true
        
    },
    cambio_aceite:{
        type:String,
        minlength:4

    },
    marca_aceite:{
        type:String,        
        minlength:4
        
    },
    kilometraje:{
        type:Int32Array,
        min:2
    },
    filtro_aceite:{
        type:String,
        min:2
    },
    original_filtro_aceite:{
        type:String,
        min:2
    },
    filtro_aire:{
        type:String,
        min:2
    }, 
    original_filtro_aire:{
        type:String,
        min:2
    },
    aceite_suspencion:{
        type:String,
        min:2
    },
    marca_aceite_suspencion:{
        type:String,
        min:2
    },
    cunas:{
        type:String,
        min:2
    },
    marca_cunas:{
        type:String,
        min:2
    },
    liquido_freno_del:{
        type:String,
        min:2
    },
    marca_liquido_freno_del:{
        type:String,
        min:2
    },
    pastillas_bandas_del:{
        type:String,
        min:2
    },
    marca_pastillas_bandas_del:{
        type:String,
        min:2
    },
    balineras_del:{
        type:String,
        min:2
    },
    marca_balineras_del:{
        type:String,
        min:2
    },
    bujes_tijera:{
        type:String,
        min:2
    },
    marca_bujes_tijera:{
        type:String,
        min:2
    },
    pastillas_bandas_tras:{
        type:String,
        min:2
    },
    marca_pastillas_bandas_tras:{
        type:String,
        min:2
    },
    balineras_tras:{
        type:String,
        min:2
    },
    marca_balineras_tras:{
        type:String,
        min:2
    },
    liquido_freno_tras:{
        type:String,
        min:2
    },
    marca_liquido_freno_tras:{
        type:String,
        min:2
    },
    lubricacion_guayas:{
        type:String,
        min:2
    },
    calibracion:{
        type:String,
        min:2
    },
    valvulas:{
        type:String,
        min:2
    },
    carburador:{
        type:String,
        min:2
    },
    correa:{
        type:String,
        min:2
    },
    marca_correa:{
        type:String,
        min:2
    },
    roles:{
        type:String,
        min:2
    },
    marca_roles:{
        type:String,
        min:2
    },
    engrase_automaticos:{
        type:String,
        min:2
    },
    observacion:{
        type:String,
        min:2
    },
    
},{
    timestamps:true
});
module.exports=mongoose.model('Mantenimiento_Completo', mantenimiento_CompletoSchema);